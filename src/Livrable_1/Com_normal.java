...
// Com-normal always do his best with only his card

// Perhaps the com_impossible will also consider others' cards

/**
 * <h1>Classe Ordinateur Moyen</h1>
 */
public class Com_normal  extends Computer_player implements GameStrategy {
	public final String c_nom="Computer normal";
	private Jest p_Jest;
	...
	/**
 	 * Constructeur de calsse ordinateur normal
 	 */
	public Com_normal() {
	...
	public Card make_offer() {
    	...
    	System.out.println("Normal Computer is choosing...");
    	try
    	{
    	    Thread.sleep(500);
    	}
			 ...
			 GM gm=GM.getinstance();
		 		...
		 		for(Iterator<Player> itpp=players_waited.iterator();itpp.hasNext();) {
		 				...
		 				Card h1=new Card();
		 		 		...
		 		 		if (p.get_hands().get(0).getCondition() == 0) {
		 		 			h1=p.get_hands().get(0);		
		 		 		} else {
			 		 		h1=p.get_hands().get(1);
		 		 		}
				...
				//new
				if(next_one ==null) {
		 			System.out.println("problem in the 120 of com normal");
		 		}else {
		 			System.out.println("problem isn't the 120 of com normal");
		 		}
     ...
     */    
     public Card take_his_card(Player p) {
		...
		dd.melanger();
 		...
 		if(p ==null) {
 			System.out.println("probleme in the 150 of com normal");
 		}else {
 			System.out.println("probleme not the 150 of com normal");
 		}
